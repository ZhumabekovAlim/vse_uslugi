...
- Требуется `passenger_id` владельца объявления. 【F:internal/taxi/http/server.go†L1589-L1607】
- После изменения статуса на `closed` сервер возвращает актуальную карточку и транслирует событие `closed` в оба WebSocket-хаба. 【F:internal/taxi/http/server.go†L1611-L1620】

### Отмена объявления водителем — `POST /api/v1/intercity/orders/{id}/cancel`

- Требуется `driver_id`, совпадающий с автором или назначенным исполнителем объявления. 【F:internal/taxi/http/server.go†L1625-L1644】
- Репозиторий обновляет статус на `closed`, проставляет `closed_at` и возвращает ошибку, если водитель не совпадает или объявление уже закрыто. 【F:internal/taxi/repo/intercity_orders.go†L299-L314】
- В ответе приходит актуальная карточка, а событие `closed` рассылается пассажирам и водителям, как и при закрытии пассажиром. 【F:internal/taxi/http/server.go†L1648-L1657】
...
- **Создание объявления** — событие `action: "created"`. Отправляется всем активным пассажирам и водителям сразу после записи объявления в БД. 【F:internal/taxi/http/server.go†L1523-L1570】
- **Закрытие объявления** — событие `action: "closed"`. Транслируется после успешного обновления статуса, включая отмену водителем. 【F:internal/taxi/http/server.go†L1589-L1657】
