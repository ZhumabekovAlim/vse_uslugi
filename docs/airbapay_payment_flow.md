...
| `subscription` | нет | Описание приобретаемой подписки исполнителя. |
| `subscription.type` | да (если `subscription` задан) | `service`, `rent` или `work`. |
| `subscription.months` | да (если `subscription` задан) | Количество месяцев, на которое продлевается подписка. |
...
При успешной оплате подписка пользователя типа `service` продлевается на три месяца от текущей даты или от даты окончания активного периода, если он ещё не истёк.
...
3. **Пополнение сервисных пакетов**
   * Для откликов продолжает использоваться `description`: слова `"отклик"/"response"` и число `10` начисляют 10 откликов (`SubscriptionRepository.AddResponsesBalance`).
   * Продление подписок оформляется отдельной целью `subscription_purchase`. Payload содержит тип (`service`, `rent`, `work`) и количество месяцев. После успешного платежа вызывается `SubscriptionRepository.ExtendSubscription`.
