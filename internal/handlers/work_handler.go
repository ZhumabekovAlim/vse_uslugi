...
func (h *WorkHandler) GetWorkByID(w http.ResponseWriter, r *http.Request) {
	...
	work, err := h.Service.GetWorkByIDWithCity(r.Context(), id, userID, cityID)
	if err != nil {
...
func (h *WorkHandler) GetWorksByUserID(w http.ResponseWriter, r *http.Request) {
	...
	works, err := h.Service.GetWorksByUserID(r.Context(), userID, cityID)
	if err != nil {
		http.Error(w, "Failed to fetch services", http.StatusInternalServerError)
		return
	}
...
func (h *WorkHandler) ServeWorkVideo(w http.ResponseWriter, r *http.Request) {
	filename := r.URL.Query().Get(":filename")
	if filename == "" {
		http.Error(w, "filename is required", http.StatusBadRequest)
		return
	...
	videoPath := filepath.Join("cmd/uploads/works/videos", filename)
...
func (h *WorkHandler) CreateWork(w http.ResponseWriter, r *http.Request) {
	...
	videoDir := "cmd/uploads/works/videos"
	...
	for _, fileHeader := range videoHeaders {
		...
		timestamp := time.Now().UnixNano()
		ext := filepath.Ext(fileHeader.Filename)
		videoName := fmt.Sprintf("work_video_%d%s", timestamp, ext)
		savePath := filepath.Join(videoDir, videoName)
		publicURL := fmt.Sprintf("/videos/works/%s", videoName)
...
func (h *WorkHandler) UpdateWork(w http.ResponseWriter, r *http.Request) {
	...
	if parsedImages, ok, err := gatherImagesFromForm[models.ImageWork](r.MultipartForm, "images", "images[]"); err != nil {
		http.Error(w, "Invalid images payload", http.StatusBadRequest)
		return
	} else if ok {
		images = parsedImages
	} else if parsedExisting, okExisting, err := gatherImagesFromForm[models.ImageWork](r.MultipartForm, "existing_images", "existing_images[]"); err != nil {
		http.Error(w, "Invalid images payload", http.StatusBadRequest)
		return
	} else if okExisting {
	...
	if parsedLinks, ok, err := gatherImagesFromForm[models.ImageWork](r.MultipartForm, "image_links", "image_links[]"); err != nil {
	...
	if len(fileHeaders) > 0 {
		...
		for _, fileHeader := range fileHeaders {
			...
			uploaded = append(uploaded, models.ImageWork{
				Name: fileHeader.Filename,
				Path: publicURL,
				Type: fileHeader.Header.Get("Content-Type"),
			})
	...
	if len(deletedImageKeys) > 0 {
		var removedImages []models.ImageWork
		images, removedImages = filterWorkImages(images, deletedImageKeys)
		if err := removeWorkImagesFromDisk(removedImages); err != nil {
			log.Printf("Failed to remove work images: %v", err)
		}
	}
	...
	if len(deletedVideoKeys) > 0 {
		...
		if err := removeWorkVideosFromDisk(removedVideos); err != nil {
			log.Printf("Failed to remove work videos: %v", err)
		}
	}
...
func filterWorkImages(images []models.ImageWork, deleteKeys []string) ([]models.ImageWork, []models.ImageWork) {
	...
	var (
		kept    []models.ImageWork
		removed []models.ImageWork
	)
...
func removeWorkImagesFromDisk(images []models.ImageWork) error {
	for _, img := range images {
		if img.Type == "link" {
			continue
		}
		if err := removeMediaFile("cmd/uploads/works", "/images/works/", img.Path); err != nil {
			return err
		}
	}
	return nil
...
func removeWorkVideosFromDisk(videos []models.Video) error {
	for _, video := range videos {
		if video.Type == "link" {
			continue
		}
		if err := removeMediaFile("cmd/uploads/works/videos", "/videos/works/", video.Path); err != nil {
			return err
		}
	}
	return nil
...
func (h *WorkHandler) GetWorkByWorkIDAndUserID(w http.ResponseWriter, r *http.Request) {
	...
	// Получение сервиса
	work, err := h.Service.GetWorkByWorkIDAndUserID(ctx, workID, userID, cityID)
	if err != nil {
