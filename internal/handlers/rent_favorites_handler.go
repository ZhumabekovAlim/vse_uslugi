...
type RentFavoriteHandler struct {
	Service *services.RentFavoriteService
}

func (h *RentFavoriteHandler) AddRentToFavorites(w http.ResponseWriter, r *http.Request) {
	var fav models.RentFavorite
	if err := json.NewDecoder(r.Body).Decode(&fav); err != nil {
		http.Error(w, "Invalid request", http.StatusBadRequest)
		return
	}
	if err := h.Service.AddRentToFavorites(r.Context(), fav); err != nil {
...
func (h *RentFavoriteHandler) RemoveRentFromFavorites(w http.ResponseWriter, r *http.Request) {
	userIDStr := r.URL.Query().Get(":user_id")
	rentIDStr := r.URL.Query().Get(":rent_id")

	userID, err1 := strconv.Atoi(userIDStr)
	rentID, err2 := strconv.Atoi(rentIDStr)
	...
	if err := h.Service.RemoveRentFromFavorites(r.Context(), userID, rentID); err != nil {
...
func (h *RentFavoriteHandler) IsRentFavorite(w http.ResponseWriter, r *http.Request) {
	userIDStr := r.URL.Query().Get(":user_id")
	rentIDStr := r.URL.Query().Get(":rent_id")
	...
	liked, err := h.Service.IsRentFavorite(r.Context(), userID, rentID)
	if err != nil {
...
func (h *RentFavoriteHandler) GetRentFavoritesByUser(w http.ResponseWriter, r *http.Request) {
	userIDStr := r.URL.Query().Get(":user_id")
	...
	favs, err := h.Service.GetRentFavoritesByUser(r.Context(), userID, cityID)
	if err != nil {
