...
func (h *PassengerHub) BroadcastEvent(event interface{}) {
	eventBytes, err := json.Marshal(event)
	if err != nil {
		h.logger.Errorf("passenger broadcast marshal failed: %v", err)
		return
	}
	...
	for _, recipient := range recipients {
		...
		if err := recipient.conn.WriteMessage(websocket.TextMessage, eventBytes); err != nil {
			h.logger.Errorf("passenger broadcast to %d failed: %v", recipient.id, err)
		}
	}
}
