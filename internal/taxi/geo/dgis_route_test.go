...
func newTestHTTPClient(t *testing.T, server *httptest.Server) *http.Client {
	...
	roundTripper, ok := baseTransport.(http.RoundTripper)
	if !ok {
		t.Fatalf("unexpected transport type: %T", baseTransport)
	}
...
func TestDGISClientRouteMatrix(t *testing.T) {
	...
	tests := []struct {
		name        string
		handler     func(t *testing.T, serverURL string) http.HandlerFunc
		wantDist    int
		wantDur     int
		wantErr     bool
		errContains string
		...
		{
			name: "redirect preserves post",
			handler: func(t *testing.T, serverURL string) http.HandlerFunc {
				redirected := false
				return func(w http.ResponseWriter, r *http.Request) {
					if !redirected {
						if r.Method != http.MethodPost {
							t.Fatalf("first request method: %s", r.Method)
						}
						if r.URL.Path != "/get_dist_matrix" {
							t.Fatalf("unexpected path on redirect: %s", r.URL.Path)
						}
						redirected = true
						w.Header().Set("Location", serverURL+"/redirected?"+r.URL.RawQuery)
						w.WriteHeader(http.StatusFound)
						return
					}
					if r.Method != http.MethodPost {
						t.Fatalf("redirected request method: %s", r.Method)
					}
					if r.URL.Path != "/redirected" {
						t.Fatalf("unexpected redirected path: %s", r.URL.Path)
					}
					if got := r.URL.Query().Get("key"); got != apiKey {
						t.Fatalf("unexpected key after redirect: %s", got)
					}
					io.Copy(io.Discard, r.Body)
					w.Header().Set("Content-Type", "application/json")
					io.WriteString(w, `{"routes":[{"status":"OK","distance":321,"duration":654}]}`)
				}
			},
			wantDist: 321,
			wantDur:  654,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			innerHandler := tt.handler(t, server.URL)
			handler = func(w http.ResponseWriter, r *http.Request) {
				if r.Method != http.MethodPost {
					t.Fatalf("unexpected method: %s", r.Method)
				}
				if r.URL.Path != "/get_dist_matrix" && r.URL.Path != "/redirected" {
