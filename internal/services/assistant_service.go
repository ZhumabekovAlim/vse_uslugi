...
func (s *AssistantService) Ask(ctx context.Context, params AskParams) (AskResult, error) {
	...
	if (!params.UseLLM || s.client == nil) && found && bestScore >= assistantConfidenceThreshold {
...
func buildSystemPrompt(role, locale string) string {
	...
	template := "" +
		...
		"5) Диплинк (если есть в контексте — выведи строкой «Перейти: ...»).\n" +
		"\n" +
		...
		"\n" +
		"МЕТАДАННЫЕ: Роль: %s. Язык: %s. Экран: %s.\n" +
