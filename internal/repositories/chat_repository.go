...
func (r *ChatRepository) GetChatsByUserID(ctx context.Context, userID int) ([]models.AdChats, error) {
	query := `
			...
			LEFT JOIN users u ON u.id = ac.performer_id
			LEFT JOIN ad_responses ar ON ar.ad_id = a.id AND ar.user_id = ac.performer_id
			WHERE a.user_id = ?
			UNION ALL
			SELECT s.id, s.name, u.id, u.name, u.surname, sr.price, sc.chat_id
			FROM service s
			LEFT JOIN service_confirmations sc ON sc.service_id = s.id
			LEFT JOIN users u ON u.id = sc.performer_id
			LEFT JOIN service_responses sr ON sr.service_id = s.id AND sr.user_id = sc.performer_id
			WHERE s.user_id = ?
			UNION ALL
			...
			LEFT JOIN users u ON u.id = rac.performer_id
			LEFT JOIN rent_ad_responses rar ON rar.rent_ad_id = ra.id AND rar.user_id = rac.performer_id
			WHERE ra.user_id = ?
			UNION ALL
			...
			LEFT JOIN users u ON u.id = wac.performer_id
			LEFT JOIN work_ad_responses war ON war.work_ad_id = wa.id AND war.user_id = wac.performer_id
			WHERE wa.user_id = ?
			UNION ALL
			SELECT r.id, r.name, u.id, u.name, u.surname, rr.price, rc.chat_id
			FROM rent r
			LEFT JOIN rent_confirmations rc ON rc.rent_id = r.id
			LEFT JOIN users u ON u.id = rc.performer_id
			LEFT JOIN rent_responses rr ON rr.rent_id = r.id AND rr.user_id = rc.performer_id
			WHERE r.user_id = ?
			UNION ALL
			SELECT w.id, w.name, u.id, u.name, u.surname, wr.price, wc.chat_id
			FROM work w
			LEFT JOIN work_confirmations wc ON wc.work_id = w.id
			LEFT JOIN users u ON u.id = wc.performer_id
			LEFT JOIN work_responses wr ON wr.work_id = w.id AND wr.user_id = wc.performer_id
			WHERE w.user_id = ?
			ORDER BY 1;
			`
