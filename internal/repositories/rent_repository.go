...
func (r *RentRepository) GetRentByID(ctx context.Context, id int, userID int) (models.Rent, error) {
   ...
   `
	...
	err := r.DB.QueryRowContext(ctx, query, userID, id).Scan(
		...
		&imagesJSON, &videosJSON, &s.CategoryID, &s.CategoryName, &subcategoryID, &subcategoryName, &subcategoryNameKz, &s.Description, &s.AvgRating, &s.Top, &s.Liked, &respondedStr, &status, &s.RentType, &s.Deposit, &lat, &lon, &s.CreatedAt,
