...
func (r *AdConfirmationRepository) Confirm(ctx context.Context, adID, performerID int) error {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE ad SET status = 'in_progress', updated_at = ? WHERE id = ?`, now, adID); err != nil {
		return err
	}
...
func (r *AdConfirmationRepository) Cancel(ctx context.Context, adID, userID int) error {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM chats WHERE id IN (SELECT chat_id FROM ad_confirmations WHERE ad_id = ?)`, adID); err != nil {
		return err
	}
	return tx.Commit()
}
...
func (r *AdConfirmationRepository) GetPerformerIDs(ctx context.Context, adID int) ([]int, error) {
	rows, err := r.DB.QueryContext(ctx, `SELECT DISTINCT performer_id FROM ad_confirmations WHERE ad_id = ?`, adID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
