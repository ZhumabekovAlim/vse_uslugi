...
func (r *AdConfirmationRepository) Confirm(ctx context.Context, adID, performerID int) error {
	...
	now := time.Now()
	if _, err = tx.ExecContext(ctx, `UPDATE ad_confirmations SET confirmed = true, status = 'in progress', updated_at = ? WHERE ad_id = ? AND performer_id = ?`, now, adID, actualPerformerID); err != nil {
		return err
	}
...
func (r *AdConfirmationRepository) Cancel(ctx context.Context, adID, userID int) error {
	...
	now := time.Now()
	if _, err := tx.ExecContext(ctx, `UPDATE ad_confirmations SET confirmed = false, status = 'cancelled', updated_at = ? WHERE ad_id = ? AND client_id = ? AND performer_id = ?`, now, adID, clientID, performerID); err != nil {
		return err
	}
	return tx.Commit()
}
