...
func (r *WorkAdRepository) GetWorkAdByWorkIDAndUserID(ctx context.Context, workadID int, userID int) (models.WorkAd, error) {
	query := `
            SELECT
              ...
              CASE WHEN sf.id IS NOT NULL THEN '1' ELSE '0' END AS liked,
              CASE WHEN sr.id IS NOT NULL THEN '1' ELSE '0' END AS responded,
              s.status, s.work_experience, u.city_id, city.name, city.type, s.schedule, s.distance_work, s.payment_period, s.latitude, s.longitude, s.created_at, s.updated_at, s.work_time_from, s.work_time_to
                FROM work_ad s
                JOIN users u ON s.user_id = u.id
	...
	err := r.DB.QueryRowContext(ctx, query, userID, userID, workadID).Scan(
		...
		&likedStr, &respondedStr, &s.Status, &s.WorkExperience, &s.CityID, &s.CityName, &s.CityType, &s.Schedule, &s.DistanceWork, &s.PaymentPeriod, &s.Latitude, &s.Longitude, &s.CreatedAt, &s.UpdatedAt, &s.WorkTimeFrom, &s.WorkTimeTo,
	)
